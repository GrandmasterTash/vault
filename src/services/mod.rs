...
///
/// As you can see, we ensure each one is traced with useful information (without leaky
/// security sensitive details like passwords or hashed).
///
#[tonic::async_trait]
impl Vault for Arc<ServiceContext> {
    ...
    #[instrument(name="api:create_password_policy", skip(self, request), fields(response.status, remote_addr=?request.remote_addr().unwrap()))]
    ...
    #[instrument(name="api:get_active_policy", skip(self, request), fields(response.status, remote_addr=?request.remote_addr().unwrap()))]
    ...
    #[instrument(name="api:make_active", skip(self, request), fields(response.status, remote_addr=?request.remote_addr().unwrap()))]
    ...
    #[instrument(name="api:get_policies", skip(self, request), fields(response.status, remote_addr=?request.remote_addr().unwrap()))]
    ...
    #[instrument(name="api:import_passwords", skip(self, request), fields(response.status, remote_addr=?request.remote_addr().unwrap()))]
    ...
    #[instrument(name="api:hash_password", skip(self, request), fields(response.status, remote_addr=?request.remote_addr().unwrap()))]
    ...
    #[instrument(name="api:validate_password", skip(self, request), fields(response.status, remote_addr=?request.remote_addr().unwrap()))]
    ...
    #[instrument(name="api:start_reset_password", skip(self, request), fields(response.status, remote_addr=?request.remote_addr().unwrap()))]
    ...
    #[instrument(name="api:complete_reset_password", skip(self, request), fields(response.status, remote_addr=?request.remote_addr().unwrap()))]
    ...
    #[instrument(name="api:delete_password", skip(self, request), fields(response.status, remote_addr=?request.remote_addr().unwrap()))]
    ...
    #[instrument(name="api:delete_passwords", skip(self, request), fields(response.status, remote_addr=?request.remote_addr().unwrap()))]
    ...
    #[instrument(name="api:get_password_types", skip(self, request), fields(response.status, remote_addr=?request.remote_addr().unwrap()))]
    ...
    #[instrument(name="api:delete_password_type", skip(self, request), fields(response.status, remote_addr=?request.remote_addr().unwrap()))]
...
fn trace_status<T>(result: Result<Response<T>, Status>) -> Result<Response<T>, Status> {
    if !tracing::Span::current().is_disabled() {
        let status = match &result {
            Ok(_)       => format!("{:?}", Code::Ok),
            Err(status) => format!("{:?}:{}", status.code(), String::from_utf8_lossy(status.details())),
        };
        tracing::Span::current().record("response.status", &status.as_str());
    }
    result
}